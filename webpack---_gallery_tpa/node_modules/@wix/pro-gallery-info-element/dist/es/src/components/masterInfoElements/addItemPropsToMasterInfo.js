...
function applyOldProduct(mas, pgItemProps) {
    if (isNarrow({
        ...
        }) && mas.enabled.Social && mas.masterType === 'internal') {
...
export default function addItemPropsToMasterInfo(pgItemProps, galleryWrapperProps, shouldUseNewSocialSharePopup, internalOrExternal, queryParams) {
    var masterConfigForItem = _.cloneDeep(galleryWrapperProps.infoElementsOptions.newInfoElementsConfig[internalOrExternal]);
    var isMigratedFromOldInfo = _.has(galleryWrapperProps, 'infoElementsOptions.newInfoElementsConfig');
    ...
    masterConfigForItem.descriptionParams.description = _.get(itemMetadata, 'description', ''); // description.split in component.
    if (!masterConfigForItem.descriptionParams.description) {
        masterConfigForItem.enabled.Description = false;
        masterConfigForItem.titleParams.paddingBottom -= pgItemProps.options.titleDescriptionSpace;
    }
    ...
    var otherPropsForSocialParams = {
        ...
        isSmallItem: isSmallItem({
            style: pgItemProps.style,
            options: _.pick(pgItemProps.options, ['cubeImages', 'cubeType', 'galleryLayout'])
        }),
        isShort: isShort({
            style: pgItemProps.style
        }),
    ...
    masterConfigForItem.socialParams = __assign(__assign({}, masterConfigForItem.socialParams), otherPropsForSocialParams);
    // masterConfigForItem.galleryLayout = galleryWrapperProps.options.galleryLayout
    overrideConfigFromQueryParamsForDebuggingPurposes(masterConfigForItem, internalOrExternal, queryParams);
...
function overrideConfigFromQueryParamsForDebuggingPurposes(master, internalOrExternal, queryParams) {
    ...
    master.order.middle = [];
    master.order.bottom = [];
    var fieldsToAlignments = getConfigFromQueryParams(queryParams, "".concat(internalOrExternal, "Alignments"));
    for (var _i = 0, _a = Object.entries(fieldsToAlignments); _i < _a.length; _i++) {
        var _b = _a[_i],
            key = _b[0],
            value = _b[1];
        // @ts-ignore
    ...
    var fieldsToSections = getConfigFromQueryParams(queryParams, "".concat(internalOrExternal, "Sections"));
    for (var _c = 0, _d = Object.entries(fieldsToSections); _c < _d.length; _c++) {
        var _e = _d[_c],
            field = _e[0],
            section = _e[1];
        // @ts-ignore
