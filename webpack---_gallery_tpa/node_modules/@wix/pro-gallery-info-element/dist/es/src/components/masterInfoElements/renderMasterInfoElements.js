...
function getMigrated(galleryWrapperProps, internalOrExternal, pgItemProps) {
    ...
    // @ts-ignore
    var res = addNewInfoElementsConfig(_.cloneDeep(pgItemProps.options), _.cloneDeep(mock.enabled), _.cloneDeep(mock.order));
    return res.newInfoElementsConfig[internalOrExternal];
}
export default function renderMasterInfoElements(type, query, galleryWrapperProps, shouldUseNewSocialSharePopup, pgItemProps, shouldControlHorizontalAlignment, shouldUseLineHeightFixer, shouldInsertDefaults) {
    var internalOrExternal = getInternalOrExternal(type);
    if (shouldInsertDefaults) {
        // @ts-ignore
        var orderAndEnabledFromUser = _.pick(galleryWrapperProps.infoElementsOptions.newInfoElementsConfig[internalOrExternal], ['order', 'enabled']);
        // const internalMock = getMasterInfo('internal')
        // const externalMock = getMasterInfo('external')
        // @ts-ignore
        var migrated = getMigrated(galleryWrapperProps, internalOrExternal, pgItemProps);
        ...
        galleryWrapperProps.infoElementsOptions.newInfoElementsConfig[internalOrExternal] = migrated;
        // @ts-ignore
    ...
    // @ts-ignore
    var masterInfoProps = addItemPropsToMasterInfo(pgItemProps, galleryWrapperProps, shouldUseNewSocialSharePopup, internalOrExternal, new Map(Object.entries(query)));
    // @ts-ignore
    return renderWixInfoComponents(masterInfoProps, shouldControlHorizontalAlignment, shouldUseLineHeightFixer);
}
...
function getInternalOrExternal(type) {
    ...
    throw new Error("MasterInfoElement does not support the externalInfoType: ".concat(type));
}
