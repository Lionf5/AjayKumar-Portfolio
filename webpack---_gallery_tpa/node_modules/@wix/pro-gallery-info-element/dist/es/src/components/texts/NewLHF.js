...
function clampInfo(_a) {
    ...
    availableHeightWithoutPaddingBottomAndMarginBottom -= verticalPaddingsAndMargins;
    log(infoToClamp, "availableHeightWithoutPaddingBottomAndMarginBottom ".concat(availableHeightWithoutPaddingBottomAndMarginBottom));
    ...
    var lineHeight = getLineHeightInPixels(getCss(infoToClamp, 'line-height', 'normal'), fontSize);
    log(infoToClamp, "clamp lineHeight: ".concat(lineHeight));
    var numOfLines = Math.floor(availableHeightWithoutPaddingBottomAndMarginBottom / lineHeight);
...
function getNumberFromPx(lineHeightValue) {
    return lineHeightValue && lineHeightValue.includes('px') ? parseInt(lineHeightValue) : 0;
}
...
function getVerticalPaddings(singleInfo) {
    ...
    if (!singleInfoStyle)
        return 0;
    return getNumberFromPx(singleInfoStyle.paddingTop) + getNumberFromPx(singleInfoStyle.paddingBottom);
}
...
function getVerticalMargins(singleInfo) {
    ...
    if (!singleInfoStyle)
        return 0;
    return getNumberFromPx(singleInfoStyle.marginTop) + getNumberFromPx(singleInfoStyle.marginBottom);
}
...
function getPaddingsAndBorder(element) {
    ...
    if (cs == null) {
        console.log('LHF warning: getComputedStyle is null, return 0 paddings and borders');
        return {
            paddingX: 0,
            paddingY: 0,
            borderX: 0,
...
function clampInfoIfNeeded(info, sectionRemainingHeight) {
    ...
    var infoFullHeight = infoHeightIncludingBordersAndPaddings + infoVerticalMargins;
    log(info, "infoHeightIncludingBordersAndPaddings: ".concat(infoHeightIncludingBordersAndPaddings, ", infoVerticalMargins: ").concat(infoVerticalMargins, ". infoFullHeight: ").concat(infoFullHeight));
    if (infoFullHeight > sectionRemainingHeight) { // not enough room for current info
        clampInfo({
            infoToClamp: info,
