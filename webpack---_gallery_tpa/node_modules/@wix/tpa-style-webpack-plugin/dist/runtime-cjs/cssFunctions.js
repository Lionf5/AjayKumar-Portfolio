...
exports.cssFunctions = exports.toRgbParts = void 0;
...
var WHITE = new tinycolor_1.TinyColor('white');
var BLACK = new tinycolor_1.TinyColor('black');
var toRgbParts = function(color) {
    if (!color) {
        throw new Error('color is empty');
    }
    var rgb = new tinycolor_1.TinyColor(color).toRgb();
    ...
    if (rgb.a === 1) {
        ...
        return {
            full: __spreadArray(__spreadArray([], rgbParts, true), [rgb.a], false).join(','),
            rgb: rgbParts.join(','),
            opacity: rgb.a,
        };
    }
};
exports.toRgbParts = toRgbParts;
exports.cssFunctions = {
    join: function(color1, strength1, color2, strength2) {
        color1 = new tinycolor_1.TinyColor(color1).toRgb();
        color2 = new tinycolor_1.TinyColor(color2).toRgb();
        //  todo: use strength
        ...
        return new tinycolor_1.TinyColor({
            r: r,
            g: g,
            b: b,
            a: a
    ...
    color: function(colorValue, tpaParams) {
        ...
        if (hexColorRegex.test(colorValue)) {
            ...
            var color = new tinycolor_1.TinyColor(colorValue);
            if (color.isValid) {
    ...
    font: function(font, tpaParams) {
        var fontValue = wixStyleFontUtils_1.wixStylesFontUtils.parseFontFromObject(font, tpaParams);
        if (fontValue) {
            var fontCssValue = wixStyleFontUtils_1.wixStylesFontUtils.toFontCssValue(fontValue);
            if (fontCssValue[fontCssValue.length - 1] === ';') {
                fontCssValue = fontCssValue.split(';')[0];
            }
            return (0, utils_1.escapeHtml)(fontCssValue);
        }
        if (typeof font === 'string' && font.indexOf('font:') === 0) {
            return font.slice(5, font.length - 1);
        }
        return (0, utils_1.escapeHtml)(font);
    },
    /*
    ...
    fontWithSize: function(font, size, tpaParams) {
        var fontValueFromObject = wixStyleFontUtils_1.wixStylesFontUtils.parseFontFromObject(font, tpaParams);
        var fontValue = fontValueFromObject ?
            __assign(__assign({}, fontValueFromObject), {
                size: size
            }) : typeof font === 'string' && font.indexOf('font:') === 0 ?
            __assign(__assign({}, wixStyleFontUtils_1.wixStylesFontUtils.parseFontFromString(font.slice(5, font.length - 1))), {
                size: size
            }) : undefined;
        if (!fontValue) {
            return (0, utils_1.escapeHtml)(font);
    ...
    fontSize: function(font, tpaParams) {
        try {
            var fontValue = wixStyleFontUtils_1.wixStylesFontUtils.parseFontFromObject(font, tpaParams) ||
                wixStyleFontUtils_1.wixStylesFontUtils.parseFontFromString(font) || {
                    size: ''
                };
            return (0, utils_1.escapeHtml)(fontValue.size);
        } catch (e) {
            return '';
        }
    },
    fontFamily: function(font, tpaParams) {
        try {
            ...
            return (0, utils_1.escapeHtml)(fontValue.family.join(','));
        } catch (e) {
            return '';
        }
    },
    opacity: function(color, opacity) {
        var oldColor = new tinycolor_1.TinyColor(color);
        var newOpacity = oldColor.toRgb().a * opacity;
        return oldColor.setAlpha(newOpacity).toRgbString();
    },
    withoutOpacity: function(color) {
        return new tinycolor_1.TinyColor(color).setAlpha(1).toRgbString();
    },
    string: function(value) {
        return (0, utils_1.escapeHtml)(value);
    },
    darken: function(colorVal, darkenValue) {
        return new tinycolor_1.TinyColor(colorVal).brighten(-1 * darkenValue * 100).toRgbString();
    },
    lighten: function(colorVal, lightenVal) {
        return new tinycolor_1.TinyColor(colorVal).lighten(lightenVal * 100).toRgbString();
    },
    whiten: function(colorVal, whitenVal) {
        return new tinycolor_1.TinyColor(colorVal).tint(whitenVal * 100).toRgbString();
    },
    ...
    unit: function(value, unit) {
        return (0, utils_1.escapeHtml)("".concat(value).concat(unit));
    },
    fallback: function() {
        var args = [];
    ...
    direction: function(value, tpaParams) {
        var direction = tpaParams.booleans[constants_1.IS_RTL_PARAM] ? 'rtl' : 'ltr';
        return constants_1.directionMap[value][direction];
    },
    ...
    calculate: function(operator) {
        ...
        if (numbersWithoutTPAParams.length > 1) {
            return "calc(".concat(numbersWithoutTPAParams.join(" ".concat(operator, " ")), ")");
        } else {
            return numbersWithoutTPAParams[0];
        }
    },
    readableFallback: function(baseColor, suggestedColor, fallbackColor) {
        var baseColorTC = new tinycolor_1.TinyColor(baseColor);
        var suggestedColorTC = new tinycolor_1.TinyColor(suggestedColor);
        return (0, tinycolor_1.isReadable)(baseColorTC, suggestedColorTC) ? suggestedColor : fallbackColor;
    },
    /**
    ...
    smartBGContrast: function(foreground, background) {
        var fgColor = new tinycolor_1.TinyColor(foreground);
        var bgColor = new tinycolor_1.TinyColor(background);
        ...
        for (var i = 0; i < luminositySteps.length; i++) {
            if (!(0, tinycolor_1.isReadable)(fgColor, bgColor)) {
