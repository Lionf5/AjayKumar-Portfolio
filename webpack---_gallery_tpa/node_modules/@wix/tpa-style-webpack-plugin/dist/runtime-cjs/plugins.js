...
var Plugins = /** @class */ (function() {
    ...
    Plugins.prototype.extractArguments = function(argsString) {
        ...
        if (result.tmpParts.length > 0) {
            throw new Error("'".concat(argsString, "' contains unbalanced parenthesis."));
        }
        return result.args;
    };
    Plugins.prototype.updateRegex = function() {
        this.regex = new RegExp("(".concat(Object.keys(this.cssFunctions).join('|'), ")\\((.*)\\)"));
    };
    return Plugins;
}());
...
function wrapWithValueProvider(fnToWrap) {
    return function() {
        ...
        return function(tpaParams) {
            return fnToWrap.apply(void 0, __spreadArray(__spreadArray([], args.map(function(fn) {
                return fn(tpaParams);
            }), false), [tpaParams], false));
        };
    };
}
