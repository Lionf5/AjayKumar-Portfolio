"use strict";
var __assign = (this && this.__assign) || function() {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s)
                if (Object.prototype.hasOwnProperty.call(s, p))
                    t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __rest = (this && this.__rest) || function(s, e) {
    var t = {};
    for (var p in s)
        if (Object.prototype.hasOwnProperty.call(s, p) && e.indexOf(p) < 0)
            t[p] = s[p];
    if (s != null && typeof Object.getOwnPropertySymbols === "function")
        for (var i = 0, p = Object.getOwnPropertySymbols(s); i < p.length; i++) {
            if (e.indexOf(p[i]) < 0 && Object.prototype.propertyIsEnumerable.call(s, p[i]))
                t[p[i]] = s[p[i]];
        }
    return t;
};
var __importDefault = (this && this.__importDefault) || function(mod) {
    return (mod && mod.__esModule) ? mod : {
        "default": mod
    };
};
Object.defineProperty(exports, "__esModule", {
    value: true
});
exports.wixStylesFontUtils = void 0;
var utils_1 = require("./utils");
var parse_css_font_1 = __importDefault(require("parse-css-font"));
exports.wixStylesFontUtils = {
    getFullFontStyles: function(_a) {
        var fontStyles = _a.fontStyles,
            siteTextPresets = _a.siteTextPresets;
        var ret = {};
        var parsedFontStyles = (0, utils_1.reduceObj)(fontStyles, function(acc, _a) {
            var key = _a.key,
                value = _a.value;
            // Fix color styles due to '.' to '-' conversion
            acc[key.replace(/\./g, '-')] = parseWixStylesFont(value);
            return acc;
        });
        var parsedSiteTextPresets = (0, utils_1.reduceObj)(siteTextPresets, function(acc, _a) {
            var key = _a.key,
                preset = _a.value;
            var presetValue = cleanWixFontValue(preset.value);
            var parsedCssFont;
            try {
                parsedCssFont = (0, parse_css_font_1.default)(presetValue);
            } catch (e) {
                parsedCssFont = (0, parse_css_font_1.default)(presetValue + ' ;');
            }
            acc[key] = __assign(__assign(__assign({}, parsedCssFont), {
                preset: key,
                editorKey: preset.editorKey
            }), (preset.displayName ? {
                displayName: preset.displayName
            } : {}));
            return acc;
        });
        // Basic template colors
        (0, utils_1.forEach)(parsedSiteTextPresets, function(preset, key) {
            return (ret[key] = parsedFontStyles[key] || preset);
        });
        // LIGHT/MEDIUM/STRONG
        ret.LIGHT = (0, parse_css_font_1.default)('12px HelveticaNeueW01-45Ligh');
        ret.MEDIUM = (0, parse_css_font_1.default)('12px HelveticaNeueW01-55Roma');
        ret.STRONG = (0, parse_css_font_1.default)('12px HelveticaNeueW01-65Medi');
        ret = __assign(__assign({}, ret), parsedFontStyles);
        (0, utils_1.forEach)(ret, function(font, key) {
            ret[key] = __assign(__assign({}, font), {
                supports: {
                    uppercase: true
                }
            });
            if (['snellroundhandw', 'niconne'].some(function(fontName) {
                    return font.family.indexOf(fontName) > -1;
                })) {
                ret[key].supports.uppercase = false;
            }
            if (ret[key].lineHeight === 'normal') {
                ret[key].lineHeight = '1.4em'; // Wix's normal line height is 1.4em...
            }
            if (ret[key].size === 'normal') {
                ret[key].size = '17px';
            }
        });
        return ret;
    },
    stringifyFontFamily: function(family) {
        var familyString = family.map(function(val) {
            return (val.indexOf(' ') > -1 ? JSON.stringify(val) : val);
        }).join(',');
        if (familyString[familyString.length - 1] === ';') {
            familyString = familyString.split(';')[0];
        }
        return familyString;
    },
    toFontCssValue: function(value) {
        var size = (0, utils_1.isNumber)(value.size) ? value.size + 'px' : value.size;
        var lineHeight = (0, utils_1.isNumber)(value.lineHeight) ? value.lineHeight + 'px' : value.lineHeight;
        var family = value.family.map(function(val) {
            return (val.indexOf(' ') > -1 ? JSON.stringify(val) : val);
        }).join(',');
        return "".concat(value.style, " ").concat(value.variant, " ").concat(value.weight, " ").concat(size, "/").concat(lineHeight, " ").concat(family);
    },
    isStringHack: function(fontParam) {
        return fontParam.fontStyleParam === false;
    },
    isValidFontParam: function(fontParam) {
        return fontParam.family !== undefined;
    },
    parseFontFromObject: function(font, tpaParams) {
        var fontValue;
        if (typeof font === 'object') {
            fontValue = font;
        } else if ((0, utils_1.isJsonLike)(font)) {
            var _a = (0, utils_1.parseJson)(font),
                theme = _a.theme,
                overrides = __rest(_a, ["theme"]);
            fontValue = __assign(__assign({
                style: '',
                variant: '',
                weight: '',
                stretch: '',
                size: '',
                lineHeight: '',
                family: []
            }, tpaParams.fonts[theme]), overrides);
        } else if (tpaParams.fonts[font]) {
            fontValue = tpaParams.fonts[font];
        }
        return fontValue;
    },
    parseFontFromString: function(font) {
        if (typeof font === 'string') {
            return (0, parse_css_font_1.default)(cleanWixFontValue(font || ''));
        }
    },
};

function cleanWixFontValue(value) {
    return value.replace(/^font\s*:\s*/, '').replace(/\s;$/, '');
}

function parseWixStylesFont(font) {
    var parsedValue;
    try {
        parsedValue = (0, parse_css_font_1.default)(cleanWixFontValue(font.value || ''));
    } catch (e) {
        parsedValue = {
            family: [],
        };
    }
    var value = '';
    if (font.style.italic) {
        value += 'italic ';
    }
    if (font.style.bold) {
        value += 'bold ';
    }
    var size = font.size || 'normal';
    if ((0, utils_1.isNumber)(size)) {
        size = "".concat(size, "px");
    }
    var lineHeight = font.lineHeight || parsedValue.lineHeight || 'normal';
    if ((0, utils_1.isNumber)(lineHeight)) {
        lineHeight = "".concat(lineHeight, "px");
    }
    value += "".concat(size, "/").concat(lineHeight, " ");
    value += font.cssFontFamily || font.family || 'NONE_EXISTS_FONT';
    var fontObj = __assign({}, (0, parse_css_font_1.default)(value));
    if (parsedValue.family && parsedValue.family.length >= fontObj.family.length) {
        fontObj.family = parsedValue.family;
    }
    fontObj.underline = font.style && font.style.underline;
    return fontObj;
}
//# sourceMappingURL=wixStyleFontUtils.js.map