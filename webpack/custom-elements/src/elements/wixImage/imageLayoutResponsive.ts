...
function computeSourceSets(measures, imageInfo, envConsts, pictureNode, shouldUseLegacyImageParallax, services) {
	...
	sourceSets.forEach((sourceSet) => {
		...
		imageInfoClone.targetHeight = shouldUseLegacyImageParallax
			? getImageHeight(measures, imageInfoClone.imageData.scrollEffect, parallaxSpeed)
			: services.getMediaDimensionsByEffect(
					imageEffectMap[sourceSet.scrollEffect],
					measures.width,
					measures.height,
					measures.screenHeight
			  ).height
...
function patch(id, measures, domNodes, imageInfo, services, envConsts, loadImage) {
	...
	const shouldUseLegacyImageParallax = !services.isExperimentOpen?.('specs.thunderbolt.useNewImageParallax')

	const extendedImageInfo = {
		...imageInfo,
		targetWidth: measures.width,
		targetHeight: shouldUseLegacyImageParallax
			? getImageHeight(measures, imageData.scrollEffect, parallaxSpeed)
			: services.getMediaDimensionsByEffect(
					imageEffectMap[imageData.scrollEffect],
					measures.width,
					measures.height,
					measures.screenHeight
			  ).height,
	...
	const compRootId = imageInfo.containerId
	if (loadImage) {
		services.imageLoader.loadImage(domNodes[compRootId], {
