...
class ImageLoader {
	...
	loadImage(compRootNode, { screenHeight, boundingRect, withScrollEffectVars }) {
		...
		if (!this.intersectionObserver || this.isImageInViewPort(boundingRect, screenHeight)) {
			this.setImageSource(compRootNode)
		} else {
			this.intersectionObserver.unobserve(compRootNode)
			this.intersectionObserver.observe(compRootNode)
		}

		if (withScrollEffectVars && this.scrollEffectsIntersectionObserver) {
			this.scrollEffectsIntersectionObserver.unobserve(compRootNode.firstElementChild)
			this.scrollEffectsIntersectionObserver.observe(compRootNode.firstElementChild)
		}
	}
	...
	setImageSource(compRootNode) {
		const wixImageNode = compRootNode.querySelector('wix-image')
