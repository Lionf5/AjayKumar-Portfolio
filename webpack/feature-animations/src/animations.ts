...
export const getAnimatorManager = (
	...
	startScrubAnimations(triggers, scrubScenes) {
		...
		function addScroll(factory: ScrollSceneFactory, source: HTMLElement, targetId: string, _effectId: string) {
		...
		function addPointer(
			...
			const scene = factory(targetId, true)
			const pointerScene = {
				isHitAreaRoot,
				effectId,
				...scene,
			...
			if (isScroll || isPointer) {
				...
				if (triggerElement) {
					...
					targetIds.forEach((target) =>
						isScroll
							? addScroll(factory as ScrollSceneFactory, triggerElement, target, effectId)
							: addPointer(
					...
					triggerElements.forEach((sourceElement: Element) => {
						...
						isScroll
							? addScroll(factory as ScrollSceneFactory, sourceElement as HTMLElement, target, effectId)
							: addPointer(
									factory as PointerSceneFactory,
