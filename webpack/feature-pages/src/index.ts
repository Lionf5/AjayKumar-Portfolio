...
const dynamicFeatureLoader = withDependencies(
	...
	(pageProvider: IPageProvider, currentRouteInfo: ICurrentRouteInfo): IPageFeatureLoader => {
		return {
			loadFeature: async <T>(featureName: FeatureName, symbol: symbol, pageId?: string) => {
				const pageIdToLoad = pageId || currentRouteInfo.getCurrentRouteInfo()?.pageId
				if (pageIdToLoad) {
					const currentPageReflector = await pageProvider(pageIdToLoad, pageIdToLoad)
					const [
						pageDynamicLoader,
