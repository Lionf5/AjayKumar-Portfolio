...
const useFunctionProps = (displayedId: string, compProps: CompProps, propsFromController: CompProps) => {
	...
	const { experiments, logger } = useContext(Context)
	...
	const updateFunctionPropsWeakMap = useCallback(
		(
			...
			const shouldYieldToMain =
				experiments['specs.thunderbolt.YieldToMainInFunctionPropsHandler'] && propName.startsWith('on')
			const functionWithEventProps = getFunctionWithEventProps(
				propName,
				prop,
				displayedId /* experiments, logger*/
			)
			...
			const functionProp = propFunctionIsOverridden
				? (...args: Array<any>) => {
						...
						return shouldYieldToMain
							? logger.runAndReport(
									() => asyncFunctionWithEventProps(...args),
									'feature-react-renderer',
									'functionPropsEventsRegistrar'
							  )
...
export const useStoresObserver = (id: string, displayedId: string): void => {
	...
	const subscribeToStores = () => {
		...
		return () => {
			compsLifeCycle.componentDidUnmount(id, displayedId)
