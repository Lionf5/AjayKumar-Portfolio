		...
		return {
			getSdkHandlers: () => ({
						...
						document: {
							height:
								document.documentElement[
									experiments['specs.thunderbolt.VeloDocumentHeightFix']
										? 'scrollHeight'
										: 'clientHeight'
								],
				...
				async openModal(url: string, options: any, compId?: string) {
					...
					// TODO: remove when specs.thunderbolt.dynamicLoadTpaFeature is merged
					if (tpaModal) {
						return tpaModal.openModal(url, options, compId ? getCompIdFromTemplateId(compId) : compId)
					}
					const pageId = compId ? tpaContextPicker.getMessageSourceContainerId({ compId })?.pageId : undefined
					const modal = await dynamicFeatureLoader.loadFeature<ITpaModal>('tpa', TpaModalSymbol, pageId)
					return modal.openModal(url, options, compId ? getCompIdFromTemplateId(compId) : compId)
				},
				...
				async openTpaPopup(url: string, options: any, compId: string) {
					...
					const pageId = tpaContextPicker.getMessageSourceContainerId({ compId })?.pageId
					const dynamicPopup = await dynamicFeatureLoader.loadFeature<ITpaPopup>(
						'tpa',
						TpaPopupSymbol,
						pageId
					)
					return dynamicPopup.openPopup(url, options, getCompIdFromTemplateId(compId))
				},
