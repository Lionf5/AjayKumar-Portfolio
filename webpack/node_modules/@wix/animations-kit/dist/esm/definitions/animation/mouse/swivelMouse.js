...
export function animate(_kit, elements, _duration, _delay, {
    ...
    return {
        progress(progressX, progressY) {
            ...
            const rotate = mapRange(0, 1, -angle, angle, progress) * inverVertical * invert;
            const [translateX, translateY] = transformOrigins[pivotAxis];
            const transform = `perspective(${perspective}px) translateX(${translateX}%) translateY(${translateY}%) ${rotateAxis}(${rotate}deg) translateX(${-translateX}%) translateY(${-translateY}%) rotate(${currentRotation}deg)`;
